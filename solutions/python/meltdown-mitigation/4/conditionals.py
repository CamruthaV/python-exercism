...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    output_efficiency = (generated_power/theoretical_max_power) * 100

    if output_efficiency >= 80:
        return 'green'
    if output_efficiency >= 60:
        return 'orange'
    if output_efficiency >= 30:
        return 'red'
    return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if total_output < 0.9 * threshold:
        return 'LOW'
    if lower_bound <= total_output <= upper_bound:
        return 'NORMAL'
    return 'DANGER'
