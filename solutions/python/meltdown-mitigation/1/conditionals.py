...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    output_range = (generated_power/theoretical_max_power) * 100

    if output_range >= 80:
        return 'green'
    elif output_range >= 60 and output_range < 80:
        return 'orange'
    elif output_range >= 30 and output_range < 60:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    total_output = temperature * neutrons_produced_per_second
    if total_output < 0.9 * threshold:
        return 'LOW'
    elif total_output < (0.1 * threshold)+threshold and total_output > threshold-(0.1 * threshold) or total_output == threshold:
        return 'NORMAL'
    else:
        return 'DANGER'
