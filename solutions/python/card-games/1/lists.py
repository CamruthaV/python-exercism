...
def approx_average_is_average(hand):
    ...
    if (hand[0] + hand[len(hand)-1]) / 2 == calc_avg or hand[len(hand) // 2] == calc_avg:
        return True
    return False
...
def average_even_is_average_odd(hand):
    ...
    even_number = hand[0::2]
    odd_number = hand[1::2]

    even_avg = sum(even_number) / len(even_number)
    odd_avg = sum(odd_number) / len(odd_number)
